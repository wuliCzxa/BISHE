...
class ImageDetection(QWidget):
    def __init__(self):
        ...
        self.flie_path = 'C:/Users/xxa/Desktop/ultralytics/z_pressure_pointer_2yolo/'  # ultralytics存放路径
        self.model_path = 'weight/weights/best.pt'    # 权重文件路径
        self.txt_path = self.flie_path + 'Result_pressure_pointer_2yolo.txt'  # 读数保存路径
    ...
    def Readvalue(self):
        ...
        x_0 = data_sorted[0][0]
        y_0 = data_sorted[0][3]
        fx_0 = data_sorted[0][2]
        fy_0 = data_sorted[0][1] + (data_sorted[0][3] - data_sorted[0][1]) * 0.25

        # 终止刻度线坐标 加入校正
        x_25 = data_sorted[1][2]
        y_25 = data_sorted[1][3]
        fx_25 = data_sorted[1][0]
        fy_25 = data_sorted[1][1] + (data_sorted[1][3] - data_sorted[1][1]) * 0.18

        # 指针端点坐标
        # 若指针目标框中心x坐标大于0刻线和结束刻度线中心x坐标和的一半，则指针在右半区，
        # 即将指针目标框的右上角坐标作为指针端点的坐标，反之则将指针目标框的左上角坐标作为指针端点的坐标
        # x_p = (data_sorted[1][0] + data_sorted[1][2]) / 2
        # y_p = (data_sorted[1][1] + data_sorted[1][3]) / 2
        x1 = (data_sorted[2][0] + data_sorted[2][2]) / 2
        x0 = (data_sorted[0][0] + data_sorted[0][2]) / 2
        x2 = (data_sorted[1][0] + data_sorted[1][2]) / 2
        if x1 > (x0 + x2) / 2:
            x_p = data_sorted[2][2]
            y_p = data_sorted[2][1]
            fx_p = data_sorted[2][0]
            fy_p = data_sorted[2][3]
        else:
            x_p = data_sorted[2][0]
            y_p = data_sorted[2][1]
            fx_p = data_sorted[2][2]
            fy_p = data_sorted[2][3]
        ...
        # 计算交点坐标
        intersection_0and25 = self.calculate_intersection(p1, p2, p3, p4)
        intersection_0andpointer = self.calculate_intersection(p1, p2, p5, p6)
        intersection_100andpointer = self.calculate_intersection(p3, p4, p5, p6)

        d1 = self.Distances(intersection_0and25, intersection_0andpointer)
        d2 = self.Distances(intersection_0and25, intersection_100andpointer)
        d3 = self.Distances(intersection_0andpointer, intersection_100andpointer)

        a1, b1 = intersection_0and25
        a2, b2 = intersection_0andpointer
        a3, b3 = intersection_100andpointer
