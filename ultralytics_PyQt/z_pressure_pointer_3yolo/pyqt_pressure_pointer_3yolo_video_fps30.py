...
class ImageDetection(QWidget):
    def __init__(self):
        ...
        self.flie_path = 'C:/Users/xxa/Desktop/ultralytics/z_pressure_pointer_3yolo/'  # ultralytics存放路径
    ...
    def initUI(self):
        ...
        self.setGeometry(0, 0, 1560, 1050)
        # 获取屏幕的尺寸
        ...
        # 创建并设置标签来显示图片
        self.imageLabel1 = QLabel('未检测视频')
        self.imageLabel1.setAlignment(Qt.AlignCenter)


        # 按钮导入视频
        self.load_button = QPushButton("导入视频")
        self.load_button.clicked.connect(self.import_video)
        ...
        # 设置定时器，定期读取文件内容
        self.timer = QTimer(self)
        self.timer.setInterval(100)  # 设置时间间隔为1000毫秒
        ...
        self.imageLabel1.setMinimumWidth(960)
        self.imageLabel1.setMaximumWidth(960)
        ...
        leftLayout.addWidget(self.load_button)
    ...
    def import_video(self):
        self.video_path, _ = QFileDialog.getOpenFileName(self, "Import Video", "", "Video Files (*.mp4 *.avi)")
        if self.video_path:
    ...
    def process_frame(self):
        ...
        # Only perform detection every 30 frames
        if self.frame_counter == 30:
            # Perform YOLO object detection
            frame_with_objects = self.Readvalue(frame)  # 自定义函数，返回带有检测结果的图像
            self.frame_counter = 0  # Reset frame counter
        else:
            frame_with_objects = frame  # Use original frame if not performing detection
        ...
        # Resize label to fit the image and display the image
    ...
    def Readvalue(self, frame):
        self.yolo = YOLO(model=self.model_path, task='detect')
        self.result = self.yolo(source=frame, save=True, conf=0.7)
        image_np = cv2.imread(self.result[0].save_dir + '/' + self.result[0].path)
        ...
        try:
            ...
            current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            self.txt_label1.setText(current_time)
            self.txt_label2.setText('未检测到读数')
